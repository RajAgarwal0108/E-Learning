...
const registerUser = asyncHandler( async(req,res) => {
    ...
    const valid = (field) => field?.trim() === "";


    if([fullname, email, username, password].some(valid) ){
        throw new ApiError(400, "All fields are required")
    };

    console.log("vallidating");
...
const loginUser = asyncHandler(async (req,res) => {
    ...
    //login TODO
    // 1. get the login info from the frontend
    // 2. Validate the format [email, password pattern or length]
    // 3. Check wheter user exists or not, if not return to register
    // 4. If user exists, then validate the email and password
    // 5. If wrong, return wrong email or password
    // 6. If correct, give access and refresh token
    // 7. Send tokens in secure cookies
...
const updateUserAvatar = asyncHandler(async(req,res) => {
    ...
    if(!avatarLocalPath){
        throw new ApiError(400, "Avatar LocalPath file not found")
    }
...
const updateCoverImage = asyncHandler(async(req,res) => {
    ...
    if(!coverImageLocalPath){
        throw new ApiError(400, "coverImage LocalPath file not found")
    }
    ...
    const user = await User.findByIdAndUpdate(
        ...
        .status(200)
        .json(new ApiResponse(
            200, user, "Updated coverImage successfully"))
